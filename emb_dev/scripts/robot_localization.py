...
worldx = 1000#508#-marker_dimension*1000 #millimeters length and width of the area
worldy = 1000 #401#-marker_dimension*1000 #millimeters
...
#    path = r'F:\System Folders\Desktop\RP_reference\RP_HW\project221020\picamera.yml' #still need to calibrate
   path = r'devel/overhead_camera/intel_camera.yml' #still need to calibrate 
...
# Start the pipeline
   ...
   while True:
    ...
    # Convert color frame to OpenCV format and detect Aruco markers
       image = np.asanyarray(color_frame.get_data())
       gray_image = cv2.cvtColor(image, cv2.COLOR_BGR2GRAY)
       data = aruco.detectMarkers(gray_image, aruco_dict) # detect aruco markers
       
       corners, ids, _ = cv2.aruco.detectMarkers(gray_image, aruco_dict, parameters=aruco_params)
       originIDglobal = 0
       if ids is not None: # if any markers were found
           ...
           if bottom_left in ids:
               ...
               print(f"bottom left ID is {ids}")
           else:
               originID = originIDglobal
               print('originID is',originID)
